...
--ファイルを一つづつ処理
function ReadText(strPathInputIndex)
	--pathの設定
	strFilePath = strPathInput1 .."/".. strPathInput[strPathInputIndex].folder.."/".. strPathInput[strPathInputIndex].file
	r = io.open(strFilePath,"r")
	--文字列の生成
	strRaw = ""
	...
	-- 頭出し
	h,f = string.find(strRaw, "###")
	strRaw = string.sub(strRaw, h)
	
	--Methodの数だけ回す
	while(h ~= nil) do
		h,f = string.find(strRaw, "###.-".."-->")
		print("className : ", strPathInput[strPathInputIndex].className, "||||||||||||||||||||||||||||||||||||||||")
		print("folder : ", strPathInput[strPathInputIndex].folder)
		print("h:"..h.." f:"..f)
		local last = f+1
		
		local strTmp = string.sub(strRaw,h,f)
		--print(string.sub(strRaw,h,f))
		
		local name = ""
		local returns = ""
		local paramaters = ""--"" is val,{} is method
		local isMethod = true
		
		--Method or Field?
		h,f = string.find(strTmp, "_returns")
		if h ~= nil then 
			isMethod = true
		else
			isMethod = false
		...
		--抽出
		name = string.match(strTmp, "_name: (.-)_\n" )
		print("name : ",name,"\n")
		if isMethod then
			returns = string.match(strTmp, "_returns: (.-)_\n" ) or ""
			paramaters = {}
			do
				local strTmpParamaters = string.match(strTmp, "_parameters: (.-)_\n" )
				print("strTmpParamaters : ",strTmpParamaters)
				
				local n = 1
				local depth = 0
				-- print(string.len(strTmpParamaters))
				while n <= strTmpParamaters:len() do
					strBit = string.sub(strTmpParamaters,n,n)
					print("depth : ",depth,", strBit : ",strBit, " n : ", n)
					if strBit == "(" then
						depth = depth + 1
					end
					if strBit == ")" then
					...
					if depth == 0 then
						-- print("Hoge!")
						if strBit == "," then
							table.insert(paramaters,string.sub(strTmpParamaters,1,n-1))
							strTmpParamaters = string.sub(strTmpParamaters,n+2)
							n = 0
						end
						if n == string.len(strTmpParamaters) then
							table.insert(paramaters,string.sub(strTmpParamaters,1,n))
						end
					end
			...
			for a = 1, #paramaters do
				print("Paramaters : ",paramaters[a])
			end
			--print(table.getn())
		else
			returns = string.match(strTmp, "_type: (.-)_\n" )
			paramaters = nil
		end
		
		print("isMethod",isMethod)
		tabInputData = {}
		tabInputData.name = name --!
		tabInputData.folder = strPathInput[strPathInputIndex].folder
...
function InputData()
	for i = 1, #strPathInput do
		ReadText(i)
	end
end

function OutputData(i)
	out = io.open("../output/"..data[i].folder.."/"..data[i].className.."_"..data[i].name.."_"..i.."_"..".sublime-snippet","w")
